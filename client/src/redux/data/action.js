...
dispatch(addTemplateDataRequest())
    return axios({
        method:"POST",
        url:"/api/template/save-template",
        headers:{
          'Authorization' : `Bearer ${token}`
        },
        data
...
dispatch(getTemplateByIdDataRequest())
    return axios({
        method:"GET",
        url:`/api/template/get-template/${id}`,
        headers:{
          'Authorization' : `Bearer ${token}`
        }
    })
